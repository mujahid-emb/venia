...
class DevServerReadyNotifierPlugin {
    constructor(devServer) {
        if (!devServer || !devServer.host || !devServer.port) {
            throw Error('Argument to DevServerReadyNotifierPlugin must be an object with host and port properties.');
        }
        this.url = url.format({
    ...
    apply(compiler) {
        const handler = once(() => setImmediate(() => console.log(boxen(chalk.green('PWADevServer ready at ' + chalk.greenBright.underline(this.url)), {
            borderColor: 'gray',
            float: 'center',
            align: 'center',
            margin: 1,
...
//# sourceMappingURL=DevServerReadyNotifierPlugin.js.map
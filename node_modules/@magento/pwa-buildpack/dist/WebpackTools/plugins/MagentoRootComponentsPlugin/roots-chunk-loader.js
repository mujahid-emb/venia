...
module.exports = async function rootComponentsChunkLoader(src) {
    ...
    try {
        ...
        const dirEntries = flatten((await Promise.all(rootsDirs.map(async dir => {
        ...
        const sources = await Promise.all(dirs.map(async dir => {
        ...
        sources.forEach(({ rootComponentPath, src }) => {
            ...
            if (errors.length) {
                console.warn(`Found PWA Studio Directive warning in ${rootComponentPath}`);
            }
            const rootComponentDirectives = directives.filter(d => d.type === 'RootComponent');

            if (rootComponentDirectives.length > 1) {
                console.warn(`Found more than 1 RootComponent Directive in ${rootComponentPath}. Only the first will be used`);
            }
            ...
            if (!rootComponentDirectives.length) {
                throw new Error(`Failed to create chunk for the following file, because it is missing a @RootComponent directive: ${rootComponentPath}`);
            }

            rootComponentMap.set(chunkNameFromRootComponentDir(dirname(rootComponentPath)), { rootDirective: rootComponentDirectives[0], rootComponentPath });
...
function generateDynamicImportCode(dirs) {
    ...
    const dynamicImportsStr = dirs.map(dir => {
...
async function asyncFilter(array, predicate) {
    const results = await Promise.all(array.map((...args) => promiseBoolReflect(predicate(...args))));
...
//# sourceMappingURL=roots-chunk-loader.js.map
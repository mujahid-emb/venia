...
class MagentoRootComponentsPlugin {
    ...
    constructor(opts = {}) {
        ...
        this.rootComponentsDirs = rootComponentsDirs || ['./src/RootComponents'];
    ...
    apply(compiler) {
        ...
        compiler.plugin('compilation', compilation => {
            compilation.plugin('normal-module-loader', (loaderContext, mod) => {
                ...
                // entry points.
                const isEntrySimpleTest = this.phase === 'development' ? mod => mod.resource.includes('/node_modules/') : mod => !mod.issuer;

                const isAnEntry = isEntrySimpleTest(mod) &&
                ...
                compilation.entries.some(entryMod => {
                    ...
                    return entryMod.dependencies.some(singleDep => singleDep.module === mod);
                });
                if (!isAnEntry) return;
        ...
        compiler.plugin('emit', (compilation, cb) => {
            ...
            const namedChunks = Array.from(Object.values(compilation.namedChunks));
            const manifest = namedChunks.reduce((acc, chunk) => {
                const { rootDirective, rootComponentPath } = rootComponentMap.get(chunk.name) || {};
            ...
            compilation.assets[this.manifestFileName] = new RawSource(JSON.stringify(manifest, null, 4));
            cb();
        });
    }
}

...
const findRedirect = message => RedirectCodes.includes(message.statusCode) && message.headers.location;
...
const emitErrorOnProtocolChange = (emit, target, redirected) => {
    ...
    if (backend.protocol === 'https:' && protocol === 'http:') {
        return emit(new Error(`pwa-buildpack: Backend domain is configured to ${target}, but redirected to unsecure HTTP. Please configure backend server to use SSL.`));
    }

    if (backend.protocol === 'http:' && protocol === 'https:') {
        return emit(new Error(`pwa-buildpack: Backend domain is configured to ${target}, but redirected to secure HTTPS. Please change configuration to point to secure backend domain: ${format(Object.assign(backend, { protocol: 'https:' }))}.`));
    }

    emit(new Error(`pwa-buildpack: Backend domain redirected to unknown protocol: ${redirected}`));
};
...
module.exports = function createDevProxy(config) {
    ...
    const proxyConf = Object.assign({
        ...
        onProxyRes(proxyRes) {
            ...
            if (redirected) {
                emitErrorOnProtocolChange(nextCallback, config.target, redirected);
            }
        },
        secure: false,
        changeOrigin: true,

...
module.exports = function createOriginSubstitutionMiddleware(oldDomain, newDomain) {
    ...
    const attributesToReplaceOrigin = ['href', 'src', 'style'].map(attr => ({
        ...
        func(node) {
            node.setAttribute(attr, node.getAttribute(attr).split(oldOrigin).join(newOrigin));
        }
    }));
    const tagsToReplaceOrigin = ['style'].map(attr => ({
        ...
        func(node) {
            ...
            stream.pipe(through(function (buf) {
                this.queue(buf.toString().split(oldOrigin).join(newOrigin));
            })).pipe(stream);
        }
    }));
    debug(`replace ${oldOrigin} with ${newOrigin} in html`, attributesToReplaceOrigin);
    const allTransforms = [...tagsToReplaceOrigin, ...attributesToReplaceOrigin];
    return harmon([], allTransforms, true);
};
//# sourceMappingURL=OriginSubstitution.js.map
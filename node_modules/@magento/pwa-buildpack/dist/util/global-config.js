...
class GlobalConfig {
    ...
    constructor(options) {
        ...
        if (typeof key !== 'function') {
            throw Error(debug.errorMsg('`key` function in options must be a function.'));
        }

        if (key.length === 0) {
            throw Error(debug.errorMsg('Provided `key` function must take at least on argument.'));
        }

        if (typeof prefix !== 'string') {
            throw Error(debug.errorMsg('Must provide a `prefix` string in options.'));
        }
    ...
    makeKey(keyparts) {
        if (keyparts.length !== this._makeKey.length) {
            throw Error(`Wrong number of arguments sent to produce unique ${this._prefix} key`);
        }
        const hash = createHash('md5');
        const key = this._makeKey(...keyparts);
        if (typeof key !== 'string') {
            throw Error(debug.errorMsg(`key function ${this._makeKey.toString()} returned a non-string value: ${key}: ${typeof key}`));
        }
        hash.update(key);
    ...
    async set(...args) {
        ...
        return new Promise((resolve, reject) => db.put(key, args.slice(-1)[0], (error, response) => error ? reject(error) : resolve(response)));
    }
    async del(...keyparts) {
        const db = await this.constructor.db();
        const key = this.makeKey(keyparts);
        return new Promise((resolve, reject) => db.del(key, (error, response) => error ? reject(error) : resolve(response)));
    }
    ...
    async clear() {
        const db = await this.constructor.db();
        return new Promise((resolve, reject) => db.clear(error => error ? reject(error) : resolve()));
    }
}

...
const tmp = () => join(__dirname, 'tmp' + Math.random().toString(20).slice(2));

const sudoPromptToRunShell = async (prompt, cmd) => {
    debug(`running "sudo ${cmd}" now...`);
    try {
        const { stdout, stderr } = await exec(`sudo -p "${escapeBashQuotes(prompt)}" ${cmd}`);
        return stdout + '\n\n' + stderr;
    } catch (e) {
        // Display all values present,
        // without a bunch of extra newlines
        const identity = x => x;
        const fullOutputForError = [e.message || e, e.stderr, e.stdout].filter(identity).join('\n\n');
...
module.exports = async (prompt, func, ...args) => {
    ...
    try {
        return await sudoPromptToRunShell(prompt, `${process.argv[0]} ${scriptLoc}`);
    } finally {
        await fs.unlink(scriptLoc);
    }
};
//# sourceMappingURL=run-as-root.js.map
...
module.exports = {
    ...
    async expired(cert) {
        return exec(`openssl x509 -checkend 0 <<< "${cert}"`).then(() => false).catch(({ stdout }) => stdout.trim() === 'Certificate will expire');
    },
    async provide(commonName) {
        if (typeof commonName !== 'string') {
            throw Error(debug.errorMsg(`Must provide a commonName to SSLCertStore.provide(). Instead, argument was ${commonName}`));
        }
    ...
    async create(commonName) {
        try {
            const cert = await runAsRoot('Creating and trusting an SSL certificate for local dev requires temporary administrative privileges.\n Enter password for %u on %H: ',
            ...
            throw Error(debug.errorMsg(`Error generating dev certificate: ${e.message} ${e.stack}`));
        }
    }
};
//# sourceMappingURL=ssl-cert-store.js.map
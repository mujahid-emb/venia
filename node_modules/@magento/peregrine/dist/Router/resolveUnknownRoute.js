...
export default function resolveUnknownRoute(opts) {
    ...
    return remotelyResolveRoute({
        ...
        return tempGetWebpackChunkData(res.type, __tmp_webpack_public_path__).then(({ rootChunkID, rootModuleID }) => ({
            rootChunkID,
            rootModuleID,
            id: res.id,
            matched: true
...
function remotelyResolveRoute(opts) {
    ...
    }).then(res => res.json()).then(res => res.data.urlResolver);
}
...
function tempGetWebpackChunkData(pageType, webpackPublicPath) {
    ...
    return fetch(new URL('roots-manifest.json', parser.href)).then(res => res.json()).then(manifest => {
        ...
        if (!firstCompatibleConfig) {
            throw new Error(`Could not find RootComponent for pageType ${pageType}`);
        }

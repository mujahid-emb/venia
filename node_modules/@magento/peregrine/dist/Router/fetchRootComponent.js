...
export default function fetchRootComponent(chunkID, moduleID) {
    return webpackInterop.loadChunk(chunkID).then(() => {
        ...
        if (!modNamespace) {
            throw new Error(`Expected chunkID ${chunkID} to have module ${moduleID}. Cannot render this route without a matching RootComponent`);
        }

        if (typeof modNamespace.default !== 'function') {
            throw new Error(`moduleID ${moduleID} in chunk ${chunkID} was missing a default export for a RootComponent`);
        }

        return modNamespace.default;
    });
}
//# sourceMappingURL=fetchRootComponent.js.map